...
the renderer depends on, and a local JSON cache keeps the last successful
month so a failed fetch still renders (project invariant: keep the previous
frame on failure).
"""
...
def load_month_cache(path: Path, year: int, month: int) -> list[WorkoutActivity]:
    ...
    if not (isinstance(payload, dict)
            ...
            and isinstance(payload.get("months"), dict)):
        print(f"Discarding legacy workout cache at {path}; it will be rebuilt.")
        return []
    entries = payload["months"].get(f"{year:04d}-{month:02d}", [])
    activities = []
...
def save_month_cache(path: Path, year: int, month: int, activities: list[WorkoutActivity]):
    """Upsert one month's activities atomically, preserving other months."""
    payload: dict = {"version": CACHE_VERSION, "months": {}}
    if path.exists():
        try:
            existing = json.loads(path.read_text())
            if (isinstance(existing, dict)
                    and existing.get("version") == CACHE_VERSION
                    and isinstance(existing.get("months"), dict)):
                payload = existing
        except (OSError, json.JSONDecodeError):
            pass  # rebuild from scratch
    payload["months"][f"{year:04d}-{month:02d}"] = [
        act.to_cache_dict() for act in activities
    ]
